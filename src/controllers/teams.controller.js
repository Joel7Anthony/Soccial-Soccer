...
Teams.updateTeam = async (req, res) => {
    const { id } = req.params;
    const { name, photo, category, series, initials, description, mail, creation_date, main_color, secondary_color, phone } = req.body;
    const newTeam = {
        name, photo, category, series, initials, description, mail, creation_date, main_color, secondary_color, phone
    };
    await pool.query("UPDATE teams set ? WHERE id =?", [newTeam, id]);
